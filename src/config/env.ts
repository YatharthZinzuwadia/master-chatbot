...
const envSchema = z.object({
  ...
  // Model configuration
  MODEL_PROVIDER: z.enum(['gemini', 'openai'], {
    errorMap: (issue, ctx) => ({
      message: 'MODEL_PROVIDER must be either "gemini" or "openai"'
    })
...
function validateEnv(): EnvConfig {
  try {
    ...
    if (env.MODEL_PROVIDER === 'gemini' && !env.GEMINI_API_KEY) {
      throw new Error('GEMINI_API_KEY is required when MODEL_PROVIDER is "gemini"');
    }
...
export function getSafeEnv(): Partial<EnvConfig> {
  const { GEMINI_API_KEY, OPENAI_API_KEY, ...safeEnv } = config;
  return {
    ...safeEnv,
